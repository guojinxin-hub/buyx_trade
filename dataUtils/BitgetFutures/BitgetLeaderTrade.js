...
class BitgetLeaderTrade {
    constructor({apiKey, secretKey, passphrase, isSimulated = false, traderId}) {
    ...
    async getTraderInfo() {
        if (!this.traderId) {
            throw new Error('traderId is required for Bitget leader trade');
        }
        return this._retry(() => this.client.getTraderInfo(this.traderId), 3, 1000);
    }

    async getTraderOrdersTrack(pageNo = 1, pageSize = 20, symbol = '') {
        if (!this.traderId) {
            throw new Error('traderId is required for Bitget leader trade');
        }
        return this._retry(() => this.client.getTraderOrdersTrack(this.traderId, pageNo, pageSize, symbol), 3, 1000);
    }

    async getTraderPerformance(period = '7d') {
        if (!this.traderId) {
            throw new Error('traderId is required for Bitget leader trade');
        }
        return this._retry(() => this.client.getTraderPerformance(this.traderId, period), 3, 1000);
    }
